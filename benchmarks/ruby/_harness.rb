...
# Normalize the non-deterministic "[<timestamp> #<pid>]" field of a formatted
# log line so the deterministic remainder (severity, progname, message) can be
# byte-compared against the Go driver.
def sample(label, line)
  printf("SAMPLE\t%s\t%s\n", label, line.sub(/\[[^\]]*\]/, "[T]"))
end
